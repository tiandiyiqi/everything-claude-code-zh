...
在 `/workflow` 启动时自动评估项目规模：

| 规模 | 判断标准 | 文档深度 | 每轮问题数 | 阶段数 |
|------|---------|---------|-----------|-------|
| 小 | 单一功能、<3 文件、<1 天工作量 | 精简版 | 3-4 题/轮 | 可跳过阶段 1-2 |
| 中 | 多模块、3-10 文件、数天工作量 | 标准流程 | 4-6 题/轮 | 完整 5 阶段 |
| 大 | 多团队、10+ 文件、周级工作量 | 完整版 | 6-10 题/轮 | 完整 5 阶段 + 深入讨论 |

### 阶段检测与切换

**自动检测机制：**
- 根据对话上下文判断当前阶段
- 检测关键词：需求/功能/用户 → 阶段 1，架构/技术选型/PRD → 阶段 2，实现/编码/计划 → 阶段 3
